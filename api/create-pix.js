...
export default async function handler(req, res) {
  ...
  try {
    ...
    // SERVER recalcula o amount baseado no bump (anti-tampering) — não confia no total do front
    const PRICE_MAIN = 47.90;
    const PRICE_BUMP = 19.90;
    const totalNum = hasBump ? (PRICE_MAIN + PRICE_BUMP) : PRICE_MAIN;
    ...
    const products = [
      { id: 'mysecret-main', name: 'My Secret 2 em 1 + Brinde', quantity: 1, price: PRICE_MAIN }
    ];
    if (hasBump) {
      products.push({ id: 'mysecret-kmed', name: 'Gel K-Med 2 em 1', quantity: 1, price: PRICE_BUMP });
    }
